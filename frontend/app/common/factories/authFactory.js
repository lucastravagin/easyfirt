(function () {
    angular.module('appEasyFit').factory('auth', [
        '$http',
        'consts',
        AuthFactory
    ])
    ...
    function AuthFactory($http, consts) {
        ...
        function submit(url, user, callback) {
            $http.post(`${consts.oapiUrl}/${url}`, user)
                .then(resp => {
                    sessionStorage.setItem(consts.userKey, JSON.stringify(resp.data))
        ...
        function logout(callback) {
            user = null
            sessionStorage.removeItem(consts.userKey)
        ...
        function getUser() {
            if (!user) {
                user = JSON.parse(sessionStorage.getItem(consts.userKey))
            }
            return user
        }
